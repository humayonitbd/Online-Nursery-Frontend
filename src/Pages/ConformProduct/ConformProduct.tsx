...
import { TConformOrder } from "@/types";
...
const ConformProduct = () => {
    ...
    const [deleteConformOrderHandler] = paymentApi.useOrdersConformProductsDeleteMutation();


    const conformOrderDeletedHandler = async (id: string) => {
      try {
        ...
        if (result.isConfirmed) {
          const res = await deleteConformOrderHandler(id).unwrap();
          if (res.success) {
            Swal.fire({
              title: "Deleted!",
              text: "Conform Order has been deleted.",
              icon: "success",
            });
          }
        }
        ...
        Swal.fire({
          title: "Error!",
          text: "Failed to delete the Confrom Order.",
          icon: "error",
        });
      }
    };
                  ...
                  <div className="overflow-x-auto">
                    <table className="table border-1 ">
                      ...
                      <tbody>
                        ...
                        {conformProducts?.data?.map(
                          (order: TConformOrder, idx: number) => (
                            <tr key={idx}>
                              <th>
                                <label>
                                  <td>{idx + 1}</td>
                                ...
                                <div className="flex items-center gap-3">
                                  <div className="avatar">
                                    <div className="mask mask-squircle h-12 w-12">
                                      <img
                                        src={order?.orderProductId?.image}
                                        alt="Conform Product"
                                      />
                                    </div>
                                  </div>
                              ...
                              <td>{order?.orderProductCategory}</td>
                              <td>{order?.orderProductId?.brand}</td>
                              ...
                              <td>
                                ...
                                <Button
                                  onClick={() =>
                                    conformOrderDeletedHandler(order?._id)
                                  }
